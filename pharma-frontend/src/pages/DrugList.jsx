...
export default function DrugList() {
  ...
  const updateProduct = useMutation({
    mutationFn: async ({ product, data }) => {
      ...
      const packagingId = product.packaging?.[0]?.id;
      if (packagingId && data.unitsPack) {
        requests.push(api.patch(`/products/${product.id}/packaging/${packagingId}`, {
          conversionToBase: Number(data.unitsPack),
        }).catch(() => {})); // safe ignore if packaging patch endpoint is separate
      }
